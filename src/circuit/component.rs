...
impl Element {
    pub fn descr(&self) -> ElementDescr {
        let (size, cells, cell_edges) = match self {
            &Element::Node => (circuit::Coords::new(0, 0), vec![(Dir::Left, 0)], None),
            &Element::Bridge => (
                circuit::Coords::new(0, 0),
                vec![(Dir::Left, 0), (Dir::Left, 0)],
                Some(vec![vec![Dir::Up, Dir::Down], vec![Dir::Left, Dir::Right]]),
            ),
            &Element::Switch(_) => (
                circuit::Coords::new(0, 0),
                vec![(Dir::Left, 0), (Dir::Right, 0)],
                Some(
                    vec![vec![Dir::Left], vec![Dir::Right, Dir::Up, Dir::Down]],
                ),
            ),
            &Element::Source => (circuit::Coords::new(0, 0), vec![(Dir::Right, 0)], None),
            &Element::Sink => (circuit::Coords::new(0, 0), vec![(Dir::Left, 0)], None),
            ...
            None => {
                ...
                cells
                    ...
                    .map(|&(cell_dir, cell_k)| {
                        ...
                        Dir::iter()
                            .filter_map(|&edge_dir| if rect.is_within(edge_dir.apply(cell_pos)) {
                                None
                            } else {
                                Some(edge_dir)
                            })
    ...
    pub fn new_component(&self, top_left_pos: circuit::Coords, rotation_cw: usize) -> Component {
        ...
        let cells = descr
            ...
            .collect();
        let cell_edges = descr
            .cell_edges
            .iter()
            .map(|edge_dirs| {
                edge_dirs
                    .iter()
                    .map(|&edge_dir| edge_dir.rotate_cw_n(rotation_cw))
                    .collect()
            })
            .collect();
...
impl Component {
    ...
    pub fn get_edge_cell_index(&self, p: circuit::Coords, dir: Dir) -> Option<usize> {
        self.cells
            .iter()
