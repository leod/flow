    ...
    pub fn can_perform(&self, circuit: &Circuit) -> bool {
        match self {
            ...
            &Action::PlaceComponent(ref component) => {
                ...
                let points_empty = component.rect.iter().all(
                    |p| !circuit.points.contains_key(&p),
                );
            ...
            &Action::RemoveComponentAtPos(pos) => {
                ...
                if let Some(component_id) = point {
                    let element = &circuit.components.get(&component_id).unwrap().element;
                    match element {
                        &Element::Input { .. } => false,
    ...
    pub fn perform(self, circuit: &mut Circuit) -> Action {
        ...
        match self {
            ...
            Action::RemoveComponentAtPos(pos) => {
                ...
                let component = circuit.components.get(&component_id).unwrap().clone();
                ...
                for (cell_index, &cell_pos) in cells {
                    ...
                    let neighbor_ids = circuit.graph.get_neighbors(cell_id).unwrap().clone();
                    for &neighbor_id in neighbor_ids.iter() {
                        let neighbor_pos = *circuit.graph.get_node(neighbor_id).unwrap();
                        let dir = Dir::from_coords(cell_pos, neighbor_pos);
                        let edge = circuit.graph.get_edge(cell_id, neighbor_id).unwrap();
            ...
            Action::RemoveEdge((id_a, cell_a), (id_b, cell_b)) => {
                let edge = circuit.graph.remove_edge((id_a, cell_a), (id_b, cell_b));
                Action::PlaceEdge((id_a, cell_a), (id_b, cell_b), edge)
            }
